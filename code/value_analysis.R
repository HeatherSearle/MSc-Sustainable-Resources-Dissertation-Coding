...
# Observed data is slight lower and more flat
...
#  mean and sd for VALUE
mean_value <- mean(monthly_data_trimmed$value_000s, na.rm = TRUE)
sd_value <- sd(monthly_data_trimmed$value_000s, na.rm = TRUE)
...
monthly_data_trimmed$standardised_monthly_value <- 
  (monthly_data_trimmed$value_000s - mean_value) / sd_value

monthly_data_trimmed$standardised_monthly_temp <- 
  (monthly_data_trimmed$avg_temp - mean_temp) / sd_temp

### running the model
SSTV.seasonal.lm <- lm(standardised_monthly_value ~ standardised_monthly_temp + month_factor,
                       data = monthly_data_trimmed)

summary(SSTV.seasonal.lm )
# adjusted r.squared = 0.86 - this is good! means majority of the variance is explained by the model

# K-fold cross validation -------------------------------------------------
# checking if the model is decent 
ctrl <- trainControl(method = "cv", number = 10)
model <- train(standardised_monthly_value ~ standardised_monthly_temp + month_factor,
               data = monthly_data_trimmed,
               method = "lm", trControl = ctrl)

summary(model) # Rsquared = 0.87
...
scenario_1_trimmed$predicted_value <- predict(SSTV.seasonal.lm, newdata = scenario_1_trimmed)
...
original_value_annual <- annual_data |>
  select(year, value_000s) |>
  mutate(source = "Observed")

predicted_value_real_annual <- scenario_1_annual |>
  select(year, total_value) |>
  mutate(source = "Predicted") |> 
  rename(value_000s = total_value)

S1_observed_predicted_annual_data <- bind_rows(original_value_annual, predicted_value_real_annual)

ggplot(S1_observed_predicted_annual_data, aes(x = year, y = value_000s, color = source)) +
  geom_line(linewidth = 1.2) +
...
# predicting landings based on the scenario 1 temperatures 
scenario_2_trimmed$predicted_value <- predict(SSTV.seasonal.lm, newdata = scenario_2_trimmed)
...
ggplot(S2_observed_predicted_data, aes(x = date, y = value_000s, color = source)) +
  geom_line(linewidth = 1.2) +
  theme_minimal() +
...
ggplot(S2_observed_predicted_annual_data, aes(x = year, y = value_000s, color = source)) +
  geom_line(linewidth = 1.2) +
  geom_smooth(se = FALSE, color = "black", size = 0.5) +
  scale_y_continuous(limits = c(0, NA)) +
  theme_minimal() +
...
# predicting landings based on the scenario 1 temperatures 
scenario_3_trimmed$predicted_value <- predict(SSTV.seasonal.lm, newdata = scenario_3_trimmed)
...
ggplot(S3_observed_predicted_data, aes(x = date, y = value_000s, color = source)) +
  geom_line(linewidth = 1.2) +
  theme_minimal() +
...
ggplot(S3_observed_predicted_annual_data, aes(x = year, y = value_000s, color = source)) +
  geom_line(linewidth = 1.2) +
  geom_smooth(se = FALSE, color = "black", size = 0.5) +
  scale_y_continuous(limits = c(0, NA)) +
  theme_minimal() +
...
ggplot(combined_scenario_data, aes(x = year, y = value_000s, color = scenario)) +
  geom_line(linewidth = 1.2) +
...
ggplot() +
  # Observed line
  geom_line(data = original_value_annual, aes(x = year, y = value_000s),
            color = "black", linewidth = 1.2) +
  
  # Predicted lines for each scenario
  geom_line(data = combined_scenario_data_trimmed, aes(x = year, y = value_000s, color = scenario),
            linewidth = 1.2) +
